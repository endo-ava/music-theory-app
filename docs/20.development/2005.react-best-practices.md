...
## 8. プロジェクト固有のベストプラクティス（Next.js + React 19）
...
### 8.4 Server Actions の利用基準

以下に該当する場合、Server Actions を優先します。

- フォーム送信、サーバー書き込みがある
- セキュリティ的にクライアントに置けない処理
- 即座に完了する比較的軽量な処理
- 非同期状態管理を簡略化したい場合
...
- [ ] 単一責任を満たしているか
- [ ] 副作用が分離されているか（useEffect を避けられない理由があるか）
- [ ] Server Component / Server Actions に移せる部分は移しているか
- [ ] Props / State の構造が適切か
- [ ] Transition の活用が適切か
- [ ] パフォーマンス最適化が過不足なく行われているか
- [ ] UI とロジックが適切に分離されているか
- [ ] a11y に配慮しているか
- [ ] エラー処理が包括的か
