...
| **`Accidental`**      | 値オブジェクト   | 変化記号（♯/♭/♮）の表現と音楽記号の管理            | 色の濃さ     |
| **`KeySignature`**    | 値オブジェクト   | 調号の状態と音名表記法の管理                       | 文書の印章   |
...
#### 2.2.6. Accidental（変化記号）

**責務**: 音楽における変化記号（♯、♭、♮）の表現と音楽記号としての機能を提供する。

**音楽理論的背景**:

- 西洋音楽における基本的な音高修飾システム
- 調号や臨時記号として楽譜に表記される
- 音名の異名同音（enharmonic equivalent）関係に関与

**主要な振る舞い**:

```typescript
class Accidental {
  readonly value: AccidentalType; // 'sharp' | 'flat' | 'natural'

  getSymbol(): string; // '♯' | '♭' | ''
  static readonly SHARP: Accidental;
  static readonly FLAT: Accidental;
  static readonly NATURAL: Accidental;
}
```

#### 2.2.7. KeySignature（調号）

**責務**: 調号の状態管理と音名表記法の統一化機能を提供する。

**音楽理論的背景**:

- 調性音楽における音名表記法の管理
- シャープ系・フラット系の自動判定ロジック
- 調号の追加順序は五度の関係に基づく
- 関係調（平行調・同主調）の体系的管理
...
```typescript
class KeySignature {
  readonly fifthsIndex: number; // 調号位置インデックス（0-11）
  readonly primaryAccidental: Accidental | null; // 主要変化記号
  readonly accidentals: ReadonlyMap<PitchClass, Accidental>; // 調号マップ

  static fromFifthsIndex(index: number): KeySignature;
  static isSharpSystem(fifthsIndex: number): boolean;
  static isFlatSystem(fifthsIndex: number): boolean;
}
```
...
graph TB
    subgraph "基本要素 (Value Objects)"
        ACC[Accidental<br/>変化記号]
        PC[PitchClass<br/>音名]
        INT[Interval<br/>音程]
        NOTE[Note<br/>絶対音高]
        KS[KeySignature<br/>調号]
...
- `value`は'sharp'、'flat'、'natural'のいずれかでなければならない
- 静的インスタンスの一意性保証（Flyweight pattern）
- getSymbol()の返値は適切なUnicode音楽記号

#### 4.1.7. KeySignatureの制約

- `fifthsIndex`は0-11の範囲内の整数
- シャープ系・フラット系の判定ロジックの一貫性
- キャッシュによるインスタンスの一意性保証（Flyweight pattern）
- `accidentals`マップの不変性とフリーズ
...
└── domain/
    ...
    │   ├── Accidental.ts        # 変化記号の定義と音楽記号表現
    │   ├── PitchClass.ts        # 音名の定義と五度圏関係
    │   ├── Interval.ts          # 音程の定義と計算
    │   ├── Note.ts              # 絶対音高の定義
    │   ├── KeySignature.ts      # 調号の管理と五度圏計算
