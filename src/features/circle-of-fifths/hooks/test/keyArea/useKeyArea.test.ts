...
// useKeyAreaBehaviorが内部で使ってる他の依存関係も同様にモック宣言
...
describe('useKeyAreaBehavior', () => {
  ...
  beforeEach(async () => {
    ...
    const { useLongPress } = await import('../../keyArea/useLongPress');
  ...
  it('正常ケース: onClickがマイナーキーでplayMinorChordAtPositionを呼び出す', async () => {
    ...
    const minorProps: UseKeyAreaBehaviorProps = {
      keyDTO: minorKeyDTO,
      segment: { ...mockSegment, position: 9 },
    };
    const { result } = renderHook(() => useKeyAreaBehavior(minorProps));
    await act(async () => {
      // マウスダウンとマウスアップのシーケンスでクリックを模擬
      result.current.handlers.onMouseDown({} as React.MouseEvent);
      result.current.handlers.onMouseUp({} as React.MouseEvent);
  ...
  // カバレッジ向上: ホバー状態のクラス名テスト（useKeyAreaBehavior.ts 34-35行目）
